    ...
    for (Tournament t : filteredTournaments) {
        ...
        for (Match m : bracketMatches) {
            if ("Final".equals(m.getGroupName()) && m.getWinnerId() != null) {
                ...
                if (champ != null) championName = champ.getTeamName();
            }
        }
...
<head>
    ...
    <style>
        ...
        .btn-logout { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #fff; padding: 0.6rem 1.5rem; text-decoration: none; border-radius: 5px; font-weight: 600; transition: all 0.3s; }
        .btn-logout:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(118,75,162,0.3); }

        /* ── Page Layout ── */
        .report-section { padding: 2.5rem 0; background-color: #f8f9fa; min-height: 80vh; }
        .page-header { text-align: center; margin-bottom: 2rem; }
        .page-title { font-size: 2.5rem; color: #1a1a2e; font-weight: 800; margin-bottom: 0.4rem; }
        .page-subtitle { font-size: 1.1rem; color: #666; }

        /* ── Report Type Tabs ── */
        .report-tabs { display: flex; gap: 0.75rem; justify-content: center; margin-bottom: 1.5rem; flex-wrap: wrap; }
        .tab-btn {
            padding: 0.55rem 1.4rem; border-radius: 50px; font-size: 0.9rem; font-weight: 600;
            border: 2px solid #667eea; cursor: pointer; text-decoration: none;
        ...
        .filter-label { font-size: 0.82rem; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.04em; }
        .filter-panel select, .filter-panel input[type="date"] {
            padding: 0.55rem 0.8rem; border: 1.5px solid #ddd; border-radius: 8px;
            font-size: 0.9rem; color: #333; background: #f9f9f9; transition: border 0.2s;
        }
        .filter-panel select:focus, .filter-panel input:focus { border-color: #667eea; outline: none; }
        .btn-generate {
            background: linear-gradient(135deg, #667eea, #764ba2); color: white;
            border: none; padding: 0.6rem 1.5rem; border-radius: 8px; font-size: 0.95rem;
            font-weight: 700; cursor: pointer; transition: all 0.3s; white-space: nowrap;
        }
        .btn-generate:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(118,75,162,0.3); }
        .btn-reset {
        ...
        .btn-reset:hover { background: #f8f9fa; color: #555; }

        /* ── Summary Stat Cards ── */
        .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem; }
        .stat-card { background: white; border-radius: 14px; padding: 1.2rem 1.4rem; box-shadow: 0 4px 16px rgba(0,0,0,0.07); }
        .stat-label { font-size: 0.82rem; color: #888; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 0.4rem; }
        .stat-value { font-size: 2rem; font-weight: 800; color: #1a1a2e; }
        .stat-icon { font-size: 1.8rem; float: right; margin-top: -0.2rem; }

        /* ── Report Table Card ── */
        .report-card { background: white; border-radius: 20px; box-shadow: 0 8px 30px rgba(0,0,0,0.08); overflow: hidden; margin-bottom: 2rem; }
        .report-card-header {
            padding: 1.2rem 1.5rem; display: flex; justify-content: space-between;
            align-items: center; border-bottom: 1.5px solid #f0f0f0; flex-wrap: wrap; gap: 0.75rem;
        }
        .report-card-title { font-size: 1.1rem; font-weight: 700; color: #1a1a2e; }
        ...
        .btn-export-csv:hover { background: #c8e6c9; }
        .btn-export-pdf {
            display: inline-flex; align-items: center; gap: 6px;
        ...
        .btn-export-pdf:hover { background: #ffcdd2; }

        /* ── Data Table ── */
        .data-table-wrap { overflow-x: auto; }
        .data-table { width: 100%; border-collapse: collapse; }
        .data-table thead tr { background: linear-gradient(135deg, #667eea, #764ba2); }
        .data-table th { padding: 0.9rem 1rem; text-align: left; color: white; font-weight: 700; font-size: 0.88rem; white-space: nowrap; }
        .data-table th:first-child { border-radius: 0; }
        .data-table td { padding: 0.85rem 1rem; border-bottom: 1px solid #f5f5f5; font-size: 0.9rem; color: #444; }
        .data-table tbody tr:hover { background: #fafafa; }
        .data-table tbody tr:last-child td { border-bottom: none; }
        .col-rank { font-weight: 800; color: #667eea; font-size: 1rem; }
        .col-name { font-weight: 600; color: #1a1a2e; }

        /* ── Category/Type badges ── */
        .badge { display: inline-block; padding: 0.2rem 0.65rem; border-radius: 20px; font-size: 0.78rem; font-weight: 700; }
        .badge-men    { background: #e3f2fd; color: #1565c0; }
        .badge-women  { background: #fce4ec; color: #880e4f; }
        .badge-mixed  { background: #e8eaf6; color: #283593; }
        .badge-indoor { background: #f3e5f5; color: #6a1b9a; }
        .badge-beach  { background: #e8f5e9; color: #1b5e20; }

        /* ── Empty State ── */
        .empty-state { text-align: center; padding: 3rem 2rem; }
        .empty-state h3 { font-size: 1.5rem; color: #2c3e50; margin-bottom: 0.75rem; }
        .empty-state p  { color: #888; font-size: 1rem; }

        /* ── Alert ── */
        .alert { padding: 1rem 1.4rem; border-radius: 12px; margin-bottom: 1.5rem; font-weight: 600; }
        .alert-info { background: #e3f2fd; border: 2px solid #1976d2; color: #0d47a1; }
        ...
        <div class="report-card">
            ...
            <div class="data-table-wrap">
                <table class="data-table">
                    ...
                    <tbody>
                        <% int i = 1; for (Map<String, String> row : summaryRows) { %>
                        <tr>
        ...
        <div class="report-card">
            ...
            <div class="data-table-wrap">
                <table class="data-table">
                    ...
                    <tbody>
                        <% int j = 1; for (Map<String, String> row : playerRows) { %>
                        <tr>
